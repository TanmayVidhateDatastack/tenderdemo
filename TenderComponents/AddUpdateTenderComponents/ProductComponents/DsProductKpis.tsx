...
const DsProductKpis: React.FC<ProductKpiProps> = ({ version }) => {
  // if (!Array.isArray(data)) {
  //   return;
  // }
  const { tenderData } = useTenderData();
  const [data, setData] = useState<TenderProduct[]>([]);

  const [totalProducts, setTotalProducts] = useState<number>(0);
  const [marginCount, setMarginCount] = useState<number>(0);
  const [totalNetValueSum, setTotalNetValueSum] = useState<number>(0);
  const [totalMarginProductSum, setTotalMarginProductSum] = useState<number>(0);
  const [totalMarginPercentage, setTotalMarginPercentage] = useState<number>(0);
  const [totalStockistDiscountSum, setTotalStockistDiscountSum] =
    useState<number>(0);
  const [totalStockistPercentage, setTotalStockistPercentage] =
    useState<number>(0);
  ...
  //   isNaN(totalStockistDiscountSum) ||
  //   isNaN(totalNetValueSum) ||
  //   totalNetValueSum === 0
  //     ? NaN
  ...
  useEffect(() => {
    if (data) {
      const marginCount = data.filter((item) => {
        const margin = Number(item?.product?.marginPercent ?? NaN);

        return !isNaN(margin) && margin < marginPercentLimit;
      }).length;
      const totalNetValueSum = data.reduce((sum, item) => {
        const netValue = item?.product?.netValue ?? null;
      ...
      setMarginCount(marginCount);
      setTotalNetValueSum(totalNetValueSum);
      setTotalMarginProductSum(totalMarginProductSum);
      setTotalMarginPercentage(
        isNaN(totalMarginProductSum) ||
          isNaN(totalNetValueSum) ||
          totalNetValueSum === 0
          ? NaN
  ...
  useEffect(() => {
    const productList = tenderData.tenderRevisions.find(
      (x) => x.version == version
    )?.tenderItems;
    setData(productList || []);
        ...
        <DsKpi
          quantity={
            <DsCurrency
              format={"IND"}
              id={"marginsum"}
              amount={parseFloat(totalMarginProductSum.toFixed(2))}
              type={"short"}
            />
          }
          // quantity={"45000Cr"}
        ...
        <DsKpi
          quantity={
            <DsCurrency
              format={"IND"}
              id={"stockitssum"}
              amount={totalStockistDiscountSum}
              type={"short"}
            />
          }
          title={"Stockist Discount"}
          subQuantity={
            isNaN(totalStockistPercentage)
              ? "0%"
              : `${totalStockistPercentage.toFixed(2)}%`
          }
