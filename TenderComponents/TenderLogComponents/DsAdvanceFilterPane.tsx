...
const DsAdvanceFilterPane: React.FC<advProps> = ({
  filters,
  onFiltersApplied,
  setIsQuickFilter,
  setFilterCount,
  ...
  const convertFilterValuesToApiFormat = (
    filterValues: any[]
  ): Record<string, any> => {
        ...
        case "value":
          if (filter.filterValues?.from || filter.filterValues?.to)
            apiFilter["tenderValue"] = {
              ...(filter.filterValues?.from && {
                min: Number(filter.filterValues.from),
              }),
              ...(filter.filterValues?.to && {
                max: Number(filter.filterValues.to),
              }),
            };
          break;
        case "status":
